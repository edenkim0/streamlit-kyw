...
with st.container():           # with절로 하나의 기능을 하는 코드를 묶어줌 (가독성 높이기)
    ...
    with col2:
        st.header("사물 검출 결과 영상")       # col2 영역 제목 
        if "processed_video" in st.session_state: # 사물 검출 완료된 비디오가 있으면 
            st.video(st.session_state["processed_video"]) # 플레이해라 
        else:
            st.write("여기에 사물 검출 결과가 표시됩니다.")

# 사물 검출 버튼 추가 
if st.button("사물 검출 실행"):   # 사물검출 실행 버튼을 누르면
    if uploaded_file is not None:  # 업로도되는 영상이 있다면
        st.session_state["processed_video"] = uploaded_file  # 검출된 영상을 사용 
        st.success("사물 검출이 완료되어 오른쪽에 표시됩니다.") # 메세지 출력 
    else: 
